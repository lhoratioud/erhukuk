...
function sanitize() {
    ...
    for (var i = 0; i < boxes.length; i++) {
        ...
        boxes[i].textContent = boxes[i].textContent.replace(/&(uuml|Uuml|ouml|Ouml|Ccedil|rsquo|acirc);/g, m => chars[m]);
        c = boxes[i].textContent;
    ...
    for (var i = 0; i < entries.length; i++) {
        ...
        entries[i].textContent = entries[i].textContent.replace(/&(uuml|Uuml|ouml|Ouml|Ccedil|rsquo|acirc);/g, m => chars[m]);
        c = entries[i].textContent;
        c = c.slice(0, c.length - 3);
        c = c.concat("...");
